const REGEXP = {
  ...
  website: /https:\/\/(www\.)facebook.com\/[-a-zA-Z0-9]+/,
...
export function validate(form, rules) {
      ...
      if (rule.min || rule.max) {
        if (rule.min && rule.max) {
          if (form[i].length < rule.min || form[i].length > rule.max) {
            errorObj[i] =
              rule.message || `Độ dài bắt buộc ${rule.min}-${rule.max}`;
            break;
          } else if (rule.min) {
            if (form[i].length < rule.min) {
              errorObj[i] =
                rule.message || `Độ dài bắt buộc lớn hơn ${rule.min} kí tự`;
              break;
            }
          } else if (rule.max) {
            if (form[i].length > rule.max) {
              errorObj[i] =
                rule.message || `Độ dài bắt buộc nhỏ hơn ${rule.max} kí tự`;
              break;
            }
          }
        }
  ...
  // if (!form.email) {
  //   errorObj.email = "Email là trường bắt buộc";
  // } else if (!/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/.test(form.email)) {
  //   errorObj.email = "Email không đúng định dạng";
  // }

  // if (!form.phone) {
  //   errorObj.phone = "Số điện thoại là trường bắt buộc";
  // } else if (!/(84|0[3|5|7|8|9])+([0-9]{8})\b/.test(form.phone)) {
  //   errorObj.phone = "Số điện thoại không đúng định dạng";
  // }

  // if (!form.website) {
  //   errorObj.website = "Facebook là trường bắt buộc";
  // } else if (
  //   !/https:\/\/(www\.)facebook.com\/[-a-zA-Z0-9]+/.test(form.website)
  // ) {
  //   errorObj.website = "Link Facebook không đúng định dạng";
  // }
